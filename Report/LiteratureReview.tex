...
\subsection{Routing in Sensor Networks - CLAIMED BY JOE}
...
The technique of clustering has a long history in increasing the efficiency of networks, originally being applied to the optimal placement of logic gates in digital circuits\cite{clustering}. Both in these applications and more modern variations, the basic principle is to group nodes of the network according to their (typically geographic) locality. Each of these clusters has a single point of contact with the rest of the network -- in a wireless sensor network this is a specific node in each cluster known as a clusterhead (CH). The benefit of adding clusters to a network is in reducing traffic as each node needs only route messages to its CH rather than all the way to its destination, and in some applications the CH can perform some aggregation or compression function\cite{1285913}\cite{aggregation} on the messages it forwards to reduce traffic between clusters. The nature of these functions is application-specific. For example, an application may require the CH to simply select the node which has forwarded the most relevant data and only forward the message from this node (if this node presented the maximum data value, say). Alternatively, the CH may take some average of all the data supplied to it, and pass on data representative of the entire cluster -- perhaps representing the mean temperature over a specific geographic region. In wireless networks, standard clustering applications are configured towards nodes sending data towards some base station, rather than communicating with arbitrary nodes in either the same or different clusters.

The nature of clustering means that a clusterhead has a significantly higher workload than other nodes in its cluster, as it must listen to every message sent from these nodes and may also have to pass on as many messages. In domains where nodes do not have permanent power source (such as WSNs), this leads to greater energy usage, causing CH batteries to drain faster\cite{placement}. As a result of this disadvantage, many modern clustering algorithms (e.g. \cite{eemc},\cite{secc}) rely on or take inspiration from the Low Energy Adaptive Clustering Hierarchy\cite{LEACH}. LEACH operates in a series of fixed-duration rounds, each beginning with a subset of nodes selecting themselves to be clusterheads. This is done with some probability related to a function of how long ago the node was last selected, giving an even distribution of CHs as the number of rounds increases. Once the CHs have selected themselves, they each broadcast an advertisement, the received strength of which used by non-CHs to determine which cluster to join for the round. Once the clusters have been formed, each CH creates a TDMA schedule for intra-cluster communications and data transmission for the round can begin. There also exists a variation of LEACH, LEACH-C, in which cluster(head) allocation is performed centrally by a base station. In both versions of the protocol, processing occurs locally and clusterheads aggregate or compress messages; both of these features reduce the number of messages needed to be sent. In accordance with the general case for clustering protocols mentioned above, CHs in LEACH can perform aggregation or compression operations that are application-specific. In their paper presenting LEACH and LEACH-C, Heinzelman \textit{et al.} found that the standard LEACH protocol performed significantly better than static clustering in terms of energy efficiency, with LEACH-C performing better still. The authors also derived an expression for the optimal number of clusters, based on the number of nodes and physical area covered by the network. However, the inclusion of both rounds and TDMA means that LEACH requires nodes in a WSN to be synchronised, which comes with significant implications in terms of operational complexity. LEACH itself offers no solutions either towards ensuring that nodes maintain synchronisation, or towards detecting if some nodes (or more likely, an entire cluster) is not performing correctly due to slipping from the prescribed TDMA schedule. However, the authors themselves suggest that a possible improvement to negate the need for TDMA is some event-based method for applications in which nodes do not generate transmissible data constantly or at prescribed intervals.
