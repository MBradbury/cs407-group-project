...
\cite{FireWxNet}
\cite{4428702} Claimed: Ivan
% http://static.usenix.org/events/mobisys06/full_papers/p28-hartung.pdf

WSN: Multi-tiered portable wireless system for monitoring weather conditions in wildland fire environments
...
FireWxNet by \citeauthor{FireWxNet} in \cite{FireWxNet} is a robust multi-tiered portable wireless system, designed to be deployed in a rugged environment. Compared to other application deployments, FireWxNet covers a unique topology which ranges from substantial and sharp elevation differences to an extremely wide coverage area of about 160 square kilometers. One of the main challenges was providing wireless communication over a wide area coverage and fine-grained local weather sensing.

Design challenges (notes)
- providing wide area communication and fine grained local weather sensing coverage. Reay data from many points of interest to base camp through an area with no internet connectivity or electricity.  
- Internet was 64km away used a small portable satellite receiver
- large elevation difference between nodes resulted in very different radio propagration models from typical flat-ground, short distance deployments.
- Some location wehere inaccessible so nodes had to be brought to the peak of the mountain by helicopter
- protection from moisture and animals - large plastic waterproof breifcase
- Concerns of overheating since temperatures could rise to 33 degrees. lack of airflow inside case as since there are no fans

Networking challenges
- Ensuring high-probability of receiving data from the sensors - Developed a best-effort converge-cast protocol. Reduces the need for every pack to reach the base station during a certain time period to only needing a single packet per node.

Deployment Issues
- Verifying the connectivity between the nodes
- Downfall in mechanism was that a user would not receive verification when adding a node to current deployed network. Neighboring nodes needs to be restarted to do so.
