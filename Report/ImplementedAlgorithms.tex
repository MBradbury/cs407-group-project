...
\begin{figure}[H]
  ...
  \begin{boxedminipage}{\linewidth}
    % 
    \null Process $j$ - \verb|eventupdate|\\
    %
    \null \textbf{variables}\\
    %
    \null\qq \% The distance we need to send information to, this is changed by the user\\
    \null\qq \var{distance}: int init 0;\\~\\
    %
    ...
    \null\qq\qq\qq \res{nhopflood}.\res{BCAST}$\langle j, \var{previous}\rangle$;\\
    \null\qq\qq \res{fi}; \\
    \null\qq\qq \res{set}($\mathit{period}$, $P_{generate}$); \\~\\
    %
    %
    \null\qq \% Receiving Change message\\
    \null\qq \emph{receive}::~\res{nhopflood}.\res{RECV}$\langle source, data\rangle \rightarrow$\\
